...
from helper import USER_AGENT, HANDICAP_BET_TYPES, get_dict_from_request
...
base_path = os.path.join('results', date)
with open(os.path.join(base_path, 'stakes.json'), 'r') as fp:
    STAKES = json.load(fp)

with open('bet_types.json', 'r') as fp:
...
text = ''
for s in STAKES:
    date_str = datetime.fromtimestamp(s['date']).strftime('%d %h, %H:%M')
    text += f'[{date_str}] {s["home"]} - {s["away"]}\n'
    for st in s['stakes']:
...
with open(os.path.join(base_path, 'result.txt'), 'w') as fp:
    fp.write(text)                
